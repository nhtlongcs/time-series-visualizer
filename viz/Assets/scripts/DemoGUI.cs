...
public class DemoGUI : MonoBehaviour
{
    ...
    void OnGUI()
    {
        ...
        if (GUI.Button(new Rect(GUIResizer.authoredScreenWidth - 190, 100, 180, 30), "  Reset", buttonStyle))
        {
            SceneManager.LoadScene(SceneManager.GetActiveScene().buildIndex);
        }

        // Quit
        if (GUI.Button(new Rect(GUIResizer.authoredScreenWidth - 190, 140, 180, 30), "  Quit ", buttonStyle))
        {
            Application.Quit();
        }
